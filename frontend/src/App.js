...
function App() {
  ...
  // Estados dinâmicos e realistas - inicializa entre 85-95%
  const [accuracy, setAccuracy] = useState(() => {
    return parseFloat((85 + Math.random() * 10).toFixed(1));
  });
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      // Accuracy varia ±0.4% entre 80-99% (mais realista)
      setAccuracy(prev => {
        const change = (Math.random() - 0.5) * 0.8;
        return Math.min(99, Math.max(80, parseFloat((prev + change).toFixed(1))));
      });
            ...
            <div className="py-4 px-4 sm:px-6 border-t border-white/5">
              <div className="max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-gray-600">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-bold text-white">Mario AI</span>
                  <span>© 2026</span>
                </div>
                <div className="flex items-center gap-4">
                </div>
