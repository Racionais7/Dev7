...
const SignalGenerator = ({ slot, onBack, platformLink }) => {
  ...
  // Function to copy game name to clipboard
  const copyGameName = async () => {
    try {
      await navigator.clipboard.writeText(slot.name);
      setIsCopied(true);
      toast.success('Nome do jogo copiado!', {
        description: `"${slot.name}" foi copiado para a área de transferência`,
        duration: 3000,
      });
      ...
      toast.error('Erro ao copiar', {
        description: 'Não foi possível copiar o nome do jogo',
      });
    }
  };
    ...
    <div className="min-h-screen bg-[#06060c] text-white overflow-hidden">
      ...
      <header className={`sticky top-0 z-50 bg-[#06060c]/90 backdrop-blur-xl border-b border-white/5 transition-all duration-300 ${isPageLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-2'}`}>
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className={`px-3 py-1.5 rounded-full transition-all duration-300 ${currentState.bg} border ${currentState.border}`}>
            <span className={`text-xs font-bold ${currentState.color}`}>
              {currentState.label}
            </span>
          </div>
        </div>
      ...
      <main className="relative z-10 max-w-5xl mx-auto px-4 py-6">
        ...
        <div className={`flex items-center gap-4 mb-6 pb-4 border-b border-white/5 transition-all duration-300 ${isPageLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-3'}`}>
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-lg font-semibold text-white truncate">{slot.name}</h1>
            <p className="text-sm text-gray-500">{providerName}</p>
          </div>
          
          {/* BOTÃO DE COPIAR - SUPER CHAMATIVO */}
          <button
            onClick={copyGameName}
            data-testid="copy-game-name-btn"
            className={`group relative flex-shrink-0 flex items-center gap-2 px-5 py-3 rounded-xl font-bold text-sm transition-all duration-300 shadow-lg ${
              isCopied 
                ? 'bg-gradient-to-r from-emerald-500 to-teal-500 text-white scale-105 shadow-emerald-500/50' 
                : 'bg-gradient-to-r from-amber-500 to-orange-500 text-white hover:scale-105 hover:shadow-amber-500/50 active:scale-95'
            }`}
          >
            {/* Brilho animado de fundo */}
            {!isCopied && (
              <div className="absolute inset-0 rounded-xl bg-gradient-to-r from-amber-400 to-orange-400 opacity-0 group-hover:opacity-100 blur transition-opacity duration-300" />
            )}
            ...
            <div className="relative flex items-center gap-2">
              {isCopied ? (
                <>
                  ...
                  <span className="font-extrabold">COPIADO!</span>
                </>
              ) : (
                <>
                  ...
                  <span className="font-extrabold">COPIAR NOME</span>
                </>
              )}
            </div>
            ...
            {/* Pulso de destaque */}
            {!isCopied && (
              <div className="absolute -inset-1 bg-gradient-to-r from-amber-500 to-orange-500 rounded-xl opacity-20 group-hover:opacity-40 blur-sm animate-pulse" />
            )}
          </button>
        </div>
        
        {/* Dica do botão de copiar - extra visível */}
        <div className={`max-w-5xl mx-auto px-4 -mt-3 mb-4 transition-all duration-300 ${isPageLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-3'}`}>
          <div className="flex items-center gap-2 text-xs text-amber-400/80 bg-amber-500/5 border border-amber-500/20 rounded-lg px-3 py-2">
            ...
            <span className="font-medium">
              💡 <strong className="text-amber-300">Dica:</strong> Use o botão laranja acima para copiar "{slot.name}" e colar na busca da plataforma!
            </span>
          </div>
        </div>
